...
# measurement
mask_train_m = file["freqency(kHz)"].isin(["1","100"]) & file["area(mm2)"].isin(["0.0050","0.0049","0.0102","0.0026","0.0025"])
mask_test_m = file["freqency(kHz)"].isin(["3","5","10","50"]) 
mask_val_m = file["freqency(kHz)"].isin(["1","100"])  & file["area(mm2)"].isin(["0.0100"])
...
x1 = tf.keras.layers.Dense(units = 10, activation='relu')(x)
output_m = tf.keras.layers.Dense(units=1)(x1)
x2 = tf.keras.layers.Dense(units = 10, activation='relu')(x)

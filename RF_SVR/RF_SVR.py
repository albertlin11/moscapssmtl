...
mdl=SVR(C=1.0, epsilon=0.2)
#mdl=RandomForestRegressor(random_state=0,max_depth=2)

mdl.fit(x_train, y_train_m)
#filepath="mlp_weights.best.hdf5"
...
y_predict_train_m=mdl.predict(x_train).reshape(-1, 1)
y_predict_train_m=scaler2.inverse_transform(y_predict_train_m)
y_predict_val_m=mdl.predict(x_val).reshape(-1, 1)
y_predict_val_m=scaler2.inverse_transform(y_predict_val_m)
y_predict_test_m=mdl.predict(x_test).reshape(-1, 1)
...
filepath1= './csv/svr_train_10.csv'
